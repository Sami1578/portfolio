...
const ToolbarButton = ({ onClick, active, disabled, title, children }) => (
  <button
    ...
    className={`rounded-md p-1.5 transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
      active ? 'bg-accent/10 text-accent' : 'text-text-muted hover:bg-border/40 hover:text-text'
    }`}
  >
    {children}
  </button>
...
/**
 * TipTap rich text editor for post content. Images are uploaded via the
 * toolbar button (or paste/drag) and inserted inline alongside text.
 */
export default function RichTextEditor({ value, onChange, uploadUrl }) {
  ...
  return (
    <div className="rounded-md border border-border bg-bg">
      <div className="flex flex-wrap items-center gap-1 border-b border-border p-2">
        ...
        <ToolbarButton title="Insert image" onClick={handleToolbarUploadClick}>
          <ImageIcon size={16} />
        </ToolbarButton>
        <span className="mx-1 h-5 w-px bg-border" />
